    ...
    if  (frame_idx % trunclen) == 0:
        ...
        for i in tracksdic.keys():
            ...
            if endfidx != -1 : #trj is dead...
                ...
                if fbr[yy,xx] == 1: 
                    Xtracks[i,:][startfidx-offset:endfidx-offset+1] = k[0] 
                    Ytracks[i,:][startfidx-offset:endfidx-offset+1] = k[1]
            else:
                Xtracks[i,:][startfidx-offset:] = k[0]
                Ytracks[i,:][startfidx-offset:] = k[1]
        ...
        savename = './mat/jay'+str(frame_idx/trunclen).zfill(3)
        savemat(savename,trk)

        #===== release memory =====
        deadtrj = np.where(array(end)!=-1)[0]
        lenoffset += len(deadtrj)
        for i in range(oldlen):
            if i in deadtrj:
                tracksdic.pop(i)
            else:
                tracksdic[i] = tracksdic[i]
                start[i] = -1
