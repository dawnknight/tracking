    ...
    if ptsidx ==0:
        ...
        if (tnum == len(tracks)) & (len(del_pts)==0) : # no pts change => simple update trj                                              
            ...
            for i in del_pts[::-1]:
                ...
                obj[j].pts[:]=obj[j].pts[0:del_idx]+[k-1 for k in obj[j].pts[del_idx:]]
                    ...
                    if sum(idxpos[:len(tracks)-Npts]):  #new pts belong to old groups                                             
                        ...
                        for ii in np.arange(len(idxpos))[idxpos]:
                            if ii >= (len(tracks)-Npts):
                                ...
                                pts[ii].pa = pa
                                Trj_idx += 1
                        #obj[pa].Trj.append( mean(obj[pa].pos,0)) ###############
                        #obj[pa].pos = []      ###################
                        #obj[pa].num = Trj_idx ####### XXXXXX ####
...
while True:
    ...
    if len(tracks) > 0:
        ...
        for d in range(len(obj)): 
            axL.plot(obj[d].xTrj,obj[d].yTrj,color = array(obj[d].Color()[::-1])/255.,linewidth=2) 
        '''
        cor = [np.int32(tr) for tr in tracks]    

        #for jj in range(len(tracks)):
            #axL.plot(cor[i]color = array(blobs[i].Color())[::-1]/255.,linewidth=2)
