...
class App:
    ...
    def run(self,bw=20,vxth=5,vyth=5):
        ...
        while (self.frame_idx <self.fnum):
            ret, pic = self.cam.read()
            frame_gray = cv2.cvtColor(pic, cv2.COLOR_BGR2GRAY)#*self.mask


            # read img in buffer
            if self.frame_idx < GOP:
                buff[self.frame_idx,:,:,:] = pic
            else:
                bind = self.frame_idx % GOP
                self.frame[:] = buff[bind]
                buff[bind] = pic
            ...
            if len(self.tracks) > 0:
                ...
                pts[pts.T[1]<0,1] = 0
                # check if feature pts in the region of interest
                ...
                if self.frame_idx >= GOP:
                    for i in range(len(self.tracks)):
                        if i not in deadidx:
                            if len(self.tracks[i])>=GOP:
                                xpts = array(self.tracks[i]).T[0][:-1]/bw
                                ypts = array(self.tracks[i]).T[1][:-1]/bw
                                vxpts = (array(self.tracks[i]).T[0][1:-1] - array(self.tracks[i]).T[0][:-2])/vxth
                                vypts = (array(self.tracks[i]).T[1][1:-1] - array(self.tracks[i]).T[1][:-2])/vyth


                    labels,cluster_center = Meanshiftcluster(tracks,bandwidth)
